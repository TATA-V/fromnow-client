...
import { RecyclerListView, DataProvider, LayoutProvider } from '@irisjae/recyclerlistview';
...
export default class CalendarScroller extends Component {
  ...
  onVisibleIndicesChanged = (all, now, notNow) => {
    const { data, numDays, numVisibleItems, visibleStartDate: _visStartDate, visibleEndDate: _visEndDate } = this.state;
  ...
  onScrollBeginDrag = () => {
    ...
    const { data, visibleStartDate, visibleEndDate, visibleStartIndex, visibleEndIndex } = this.state;
    const prevStartDate = visibleStartDate ? visibleStartDate : data[visibleStartIndex] ? data[visibleStartIndex].date : moment();
    const prevEndDate = visibleEndDate ? visibleEndDate : data[visibleEndIndex] ? data[visibleEndIndex].date : moment();
  ...
  rowRenderer = (type, data, i, extState) => {
    ...
    return <View style={{ marginRight: 5 }}>{renderDay({ ...data, ...extState })}</View>;
  };
  ...
  render() {
    ...
    return (
      <View className="flex-1 h-[70px]" onLayout={this.onLayout}>
        <RecyclerListView
