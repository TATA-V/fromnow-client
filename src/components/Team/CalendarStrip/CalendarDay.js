...
class CalendarDay extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.datesBlacklist !== this.props.datesBlacklist || prevProps.datesWhitelist !== this.props.datesWhitelist || hasDateChanged) {
  ...
  renderDots(marking) {
    ...
    validDots = marking.dots
      ...
      .map((dot, index) => {
        ...
        return <View key={dot.key || formattedDate + index} style={[baseDotStyle, { backgroundColor }, markedDatesStyle]} />;
      });
  ...
  renderLines(marking) {
    ...
    validLines = marking.lines
      ...
      .map((line, index) => {
        ...
        return <View key={line.key ? line.key : index} style={[baseLineStyle, { backgroundColor, width }, markedDatesStyle]} />;
      });
  ...
  render() {
    ...
    const { enabled, selected, containerHeight, containerWidth, containerBorderRadius, customStyle, dateNameFontSize, dateNumberFontSize } =
      this.state;
    ...
    let _dateViewStyle = enabled ? [{ backgroundColor: 'transparent' }] : [{ opacity: disabledDateOpacity }];
    ...
    if (enabled && selected) {
      ...
      if (styleWeekend && (date.isoWeekday() === 6 || date.isoWeekday() === 7)) {
      ...
      _dateNameStyle = [styles.dateName, highlightDateNameStyle, _customHighlightDateNameStyle];
      _dateNumberStyle = [styles.dateNumber, highlightDateNumberStyle, _customHighlightDateNumberStyle];
    ...
    let containerStyle = selected ? { ...dayContainerStyle, ...highlightDateContainerStyle } : dayContainerStyle;
    ...
    if (DayComponent) {
      ...
      day = (
        <TouchableOpacity className="bg-slate-400 h-[70px]" onPress={onDateSelected.bind(this, date)} disabled={!enabled}>
          <View style={[styles.dateContainer, responsiveDateContainerStyle, _dateViewStyle, containerStyle]}>
            <View className="space-y-[6px] flex items-center">
              {showDayNumber && (
                <View style={_dateNumberContainerStyle}>
                  <Text style={[{ fontSize: dateNumberFontSize }, _dateNumberStyle]} allowFontScaling={allowDayTextScaling}>
                    {date.date()}
                  </Text>
                </View>
              )}
              {showDayName && (
                <Text style={[{ fontSize: dateNameFontSize }, _dateNameStyle]} allowFontScaling={allowDayTextScaling}>
                  {date.format('ddd')}
                </Text>
              )}
              {this.renderMarking()}
    ...
    return calendarAnimation && !scrollable ? (
      <Animated.View style={[styles.dateRootContainer, { opacity: this.state.animatedValue }]}>{day}</Animated.View>
    ) : (
